...
### 1. 게임 화면 선택 애니메이션 성능 개선 (최우선)

- 대상은 `src/app/play-game/[id]/page.tsx`와 `src/hooks/useGameSelectionAnimation.ts`의 후보 선택·다음 대결 전환 흐름이다.
- 현재 좌·우 카드를 각각 최대 400px/2000px 이동시키고 고정된 1초 `setTimeout` 뒤 다음 후보를 표시한다. 저사양 모바일에서 프레임 저하가 있는지 먼저 측정하고, 애니메이션 완료 시점과 화면 전환 시점을 일치시키는 방향으로 개선한다.
- `transform` 중심 렌더링을 유지하고 불필요한 재렌더, 중복 spring 실행, 큰 이동 거리와 무거운 그림자·필터의 합성 비용을 점검한다.
- 사용자가 연속으로 선택할 수 없게 하는 현재 잠금 동작, 승자·패자 계산, 다음 라운드 요청 순서와 시각적 방향은 변경하지 않는다.
- `prefers-reduced-motion` 사용자는 이동 효과를 최소화하고 지연 없이 다음 대결로 진행할 수 있게 한다.
- 데스크톱과 모바일에서 선택 반응, 다음 후보 표시, 결승 이동을 직접 확인하고 React Profiler/브라우저 Performance 기록으로 개선 전후를 비교한다.
- 완료 기준은 선택 직후 입력 반응이 유지되고, 애니메이션 중 긴 메인 스레드 작업과 눈에 띄는 끊김이 없으며, 기존 게임 도메인 테스트와 lint/typecheck/test/build가 모두 통과하는 것이다.
