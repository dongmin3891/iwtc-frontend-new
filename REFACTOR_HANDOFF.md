...
- 현재 NestJS 백엔드에는 `DELETE /api/me/game-manage/world-cups/{worldCupId}` endpoint 자체가 없다. 프론트 변경만 배포하면 파싱 오류 대신 HTTP 404가 발생하므로 월드컵 삭제 기능 완료로 취급하지 않는다.
- 백엔드의 영구 삭제 범위가 승인되고 endpoint가 구현된 뒤 프론트 전체 검증을 실행한다. 실제 화면에서는 요청에 body가 없는지, 성공 응답이 204인지, 목록 캐시 무효화 후 카드가 사라지는지, 실패 시 카드가 유지되며 오류가 표시되는지, 401 갱신 흐름이 유지되는지 확인한다.
- 후보·댓글 DELETE도 본문 없는 요청으로 바뀌므로 기존 endpoint와의 회귀 테스트를 함께 실행한다. 백엔드 구현 전에는 운영 월드컵으로 실제 삭제를 검증하지 않는다.
