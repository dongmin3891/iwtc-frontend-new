...
- 마지막 코드 커밋은 이 문서가 포함된 최신 HEAD다. 정확한 해시는 `git log -1 --oneline`으로 확인한다.
- 자동 배포 커밋은 기능 커밋 push 후 GitHub Actions가 생성하므로 `git log -5 --oneline`과 Actions 실행 결과를 함께 확인한다.
...
### 2026-09-21 게임 고정 대진 전환

- 기존 게임은 매 라운드 종료 시 지금까지의 탈락자 ID만 백엔드에 보내고, 백엔드는 탈락자를 제외한 전체 공개 후보를 다시 섞어 다음 라운드 인원만큼 반환했다.
- 전체 공개 후보 수가 사용자가 고른 시작 라운드보다 많으면 최초 대진에 참가하지 않은 후보가 다음 라운드에 들어오고 실제 승자가 탈락하는 결함이 있었다. 예를 들어 후보 8명 중 4강을 시작하면 최초 4명 밖의 후보가 결승에 등장할 수 있었다.
- 게임 시작 시에만 선택 라운드 수만큼 후보를 조회하고, 이후에는 프론트엔드의 `roundWinners`에 각 경기 승자 객체를 누적한다. 현재 라운드 마지막 경기 후 이 승자 배열을 그대로 다음 라운드 대진으로 사용한다.
- 라운드 사이의 `/world-cups/{id}/contents` 재요청과 프론트의 `excludeContentsIds` 전달을 제거했다. 백엔드 API 계약과 DTO는 변경하지 않았으며 초기 대진 조회에는 기존 endpoint를 그대로 사용한다.
- `src/domain/game/play.test.ts`에 8강의 선택 결과 `[1, 4, 5, 7]`만 4강으로 올라가고, 다시 선택한 `[4, 7]`만 결승으로 올라가는 회귀 테스트를 추가했다.
- 변경 파일은 `src/app/play-game/[id]/page.tsx`, `src/domain/game/play.ts`, `src/domain/game/play.test.ts`, `src/services/WorldCupService.ts`다.
- 전체 테스트 76개, `npm run typecheck`, `npm run lint`, `npm run build`를 통과했다. 린트·빌드에는 기존 관리 화면 `<img>` 경고 2건과 Browserslist 갱신 안내만 남아 있다.
- push 후 자동 배포가 완료되면 후보 수가 16명인 월드컵에서 4강 또는 8강을 선택해, 매 라운드에서 실제로 선택한 후보만 다음 라운드와 결승에 남는지 운영 검증한다.

### 다음 작업: 인게임 인터랙션 보완 (검토 완료, 미구현)

- 운영 `https://iwtc.ddongmy.com/play-game/13`에서 4강을 직접 실행해 확인했다. 선택 클릭과 다음 후보 DOM 교체는 동작했지만 진행률과 전환 연출에 문제가 있다. 이 검증은 고정 대진 기능 배포 전 운영 코드에서 진행했다.
- 진행률은 최초 후보 목록을 적용할 때부터 `advanceProgress`를 호출해 4강 시작 직후 33%, 첫 선택 후 67%, 결승 진입 시 100%로 한 경기씩 앞선다. 목록 적용과 진행률 증가를 분리하고 실제 선택 완료 시점에만 `완료 경기 수 / (초기 라운드 - 1)`로 계산해야 한다.
- `animated.button`에 Tailwind `transition`과 `hover:-translate-y-1`이 있고 React Spring도 같은 요소의 `transform`·`opacity`를 제어한다. CSS 전환과 spring을 서로 다른 래퍼로 분리하거나 CSS transition 대상에서 transform·opacity를 제외한다.
- 현재 순서는 선택 애니메이션 완료 → 기존 카드 spring 즉시 초기화 → 다음 후보 데이터 적용이다. 느린 기기에서는 사라진 기존 카드가 원위치에서 잠깐 다시 보일 수 있으므로, 카드가 숨겨진 상태에서 후보를 교체하고 새 카드의 진입 상태를 초기화한 뒤 나타내는 순서로 바꾼다.
- 선택 카드는 320ms 동안 32px 이동·1.02 확대만 되어 선택 피드백이 약하다. 선택 즉시 테두리·체크·선택됨 상태를 표시하고 탈락 카드 퇴장 후 새 후보를 짧게 진입시키는 2단계 전환을 권장한다.
- 모바일도 데스크톱과 같은 x축 이동을 사용하지만 카드가 세로로 배치된다. 데스크톱은 좌우 이동, 모바일은 페이드·축소 또는 세로 방향 전환으로 분리한다.
- `prefers-reduced-motion`에서는 모든 시간이 0이어서 시각 피드백이 사라진다. 이동은 제거하되 테두리·체크 같은 비모션 선택 피드백은 유지한다.
- 비로그인 상태 헤더의 `내 월드컵` 링크가 `/members//games`를 prefetch해 운영 콘솔에 404가 한 건 발생한다. 게임 선택 동작과 직접 관련은 없지만 별도 수정 대상이다.
- 인터랙션 보완 시 데스크톱과 390px 모바일에서 좌·우 선택, 동일 카드 빠른 연속 클릭 방지, reduced-motion, 결승 결과 이동을 확인하고 실제 픽셀 렌더링과 computed style까지 검증한다.
...
- 후보 선택 시 승자·패자 ID를 계산하고, 현재 라운드의 승자 객체를 누적하는 로직을 순수 함수로 분리했다.
- 결승 종료·고정된 승자 목록으로 다음 라운드 시작·같은 라운드의 다음 후보 표시를 상태 전이로 분리했다.
