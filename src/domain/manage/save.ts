...
export const saveWorldCupContentChanges = async (
    ...
    requests.push(...deleteList.map((item) => dependencies.removeContent(worldCupId, item.contentsId, accessToken)));
    requests.push(
        ...modifyList
            .filter((item) => !deletedContentIds.has(item.contentsId))
            .map((item) => {
                if (item.fileType === 'file') {
                    return dependencies.updateStaticContent({
                        worldCupId,
                ...
                return dependencies.updateContent(
                    worldCupId,
                    item.contentsId,
    ...
    if (newList.length > 0) {
        requests.push(createNewWorldCupContents({ worldCupId, accessToken, contents: newList }, dependencies));
    }
